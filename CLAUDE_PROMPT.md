...
| `dream` | **对话开头 breath 之后**调用一次——读最近记忆；有洞察时生成待审核提案 |
| `review_proposals` | 查看、展示详情、批准或拒绝自动合并与 Dream 洞察提案 |
...
- `delete=True`：把桶移入隐藏回收区，不再参与召回
- `restore=True`：恢复最近一次软删除的桶
- `history=True`：查看审计事件；`revision=事件ID`：回滚到该事件保存的版本
- `scope="project:..."`：把写入和召回严格限制在指定作用域，不会回退全局
- 普通检索只记录命中与返回；确认记忆确实有用时用 `trace(bucket_id, confirm=True)` 增加热度
...
有沉淀时使用 `dream(insight="...", source_buckets="id1,id2")` 生成待审核洞察提案。
不要仅凭 Dream 推测直接修改、resolve 或删除事实记忆。
- 没有沉淀就不写，不强迫产出
